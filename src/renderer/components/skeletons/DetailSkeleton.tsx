...
export const DetailSkeleton: React.FC<DetailSkeletonProps> = ({ onBack }) => {
  return (
    <div>
      ...
      <div className="flex gap-2 px-6 pt-3 pb-3 overflow-x-auto scrollbar-none">
        {[1, 2, 3].map((i) => (
          <Skeleton key={i} className="h-6 w-14 rounded-md flex-shrink-0" />
        ))}
      </div>

      <div className="px-6 pt-3 pb-6 flex flex-col gap-3">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-lg border border-neutral-800/50 bg-neutral-900/40">
